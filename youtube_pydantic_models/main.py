...
from youtube_pydantic_models.quotas import DEFAULT_LIMIT_PER_DAY, Quotas
...
class YoutubeClient:
    def __init__(
        ...
        self._api_base_path: str = "https://www.googleapis.com/"
        self._api_path: str = (
            f"{self._api_base_path}{self._api_service_name}/{self._api_version}/"
        )
    ...
    def set_quota_per_day(self, new_quota: int) -> None:
        if new_quota is not int or new_quota <= 0:
    ...
    def get_channel(self, id: str, part: str) -> YoutubeChannelResource | None:
        """
    ...
    def get_video(self, id: str, part: str) -> YoutubeVideoResource | None:
        """
        ...
        response = self._get_request(
            endpoint="videos", params=params, quota=self._quotas_table.LIST_VIDEO.value
        )
    ...
    def get_playlists(
        ...
        ------
            QuotaException : unavailable YouTube quota
        """
        params = {"channelId": channel_id, "part": part}
        if id:
            params.update({"id": id})
        if max_results:
        ...
        if response and response["pageInfo"]["totalResults"] > 0:
            return [YoutubePlaylistResource(**item) for item in response["items"]]
        return None

    def search(
        self, channel_id: str, part: str, type: str
        ...
        ------
            QuotaException : unavailable YouTube quota
        """
        params = {"channelId": channel_id, "part": part, "type": type}

        response = self._get_request(
            endpoint="search", params=params, quota=self._quotas_table.SEARCH.value
        )

        if response and response["pageInfo"]["totalResults"] > 0:
            return [YoutubeSearchResource(**item) for item in response["items"]]
        return None
    ...
    def iter_playlists(
        ...
        while True:
            response = self._get_request(
                endpoint="playlists", params=base_params, quota=request_quota
            )
    ...
    def _get_request(
        self, endpoint: str, params: dict, quota: int
    ) -> dict | list | None:
        if not self._check_available_quota(quota):
            raise QuotaException()

        response = requests.get(
            self._api_path + endpoint, params=self._set_key_param(params)
        )
