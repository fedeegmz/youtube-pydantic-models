...
class TestGetVideo(YoutubeClientForTesting):
    def test_get_available_video(self):
        ...
        data = video.model_dump(by_alias=True, exclude_none=True)
    ...
    def test_get_available_video_with_available_part(self):
        video_id = "PJm8WNajZtw"
        video = self.client.get_video(
            id=video_id, part="snippet, contentDetails, player, recordingDetails"
        )
        data = video.model_dump(by_alias=True, exclude_none=True)
        ...
        assert (
            len(data) == 7
        )  # kind, etag, id, snippet, contentDetails, player, recordingDetails

    def test_get_available_video_with_unvailable_part(self):
        video_id = "PJm8WNajZtw"
        video = self.client.get_video(id=video_id, part="wrong, player, localizations")
