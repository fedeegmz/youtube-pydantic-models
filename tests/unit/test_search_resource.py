...
class TestSearchChannelResource(TestYoutubeResource):
    def setUp(self):
        params = self.init_params(
            "search_channel_example_data.json", YoutubeSearchResource
        )
...
class TestSearchPlaylistResource(TestYoutubeResource):
    def setUp(self):
        params = self.init_params(
            "search_playlist_example_data.json", YoutubeSearchResource
        )
        self.json_data = params[0]
        self.model_data = params[1]
...
class TestSearchVideoResource(TestYoutubeResource):
    def setUp(self):
        params = self.init_params(
            "search_video_example_data.json", YoutubeSearchResource
        )
        self.json_data = params[0]
        self.model_data = params[1]
